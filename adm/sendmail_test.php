...
if (isset($_POST['email'])) {
    ...
    for ($i=0; $i<count($email); $i++){
        ...
        $real_email[] = $email[$i];
        mailer($config['cf_admin_email_name'], $config['cf_admin_email'], trim($email[$i]), '[제목]안녕하세요 이성택 입니다', '<span style="font-size:9pt;">메일내용 연습중입니다.<p>이 내용이 제대로 보인다면 보내는 메일 서버에는 이상이 없는것입니다.<p>'.G5_TIME_YMDHIS.'<p>이 메일 주소로는 회신되지 않습니다.</span>', 1);
    }
